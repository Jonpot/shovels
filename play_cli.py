        ...
        if current_player.id in human_ids:
            ...
            try:
                if base == "quit":
                    ...
                    print(
                        "  action <char_idx> <num_cards> <suit> - e.g. action 0 2 clubs"
                    )
                    ...
                    if suit == Suit.CLUBS:
                        ...
                        target_info = {
                            "target_player_id": t_p,
                            "target_char_index": t_c,
                        }
                    ...
                    if state.dug_cards:
                        ...
                        indices_str = input(
                            "Indices to use from DUG (comma separated): "
                        )
                        dug_indices = (
                    ...
                    if char.suit == Suit.CLUBS:
                        ...
                        for _ in range(limit):
                            inp = input(
                                f"Target {len(targets) + 1} (p_id char_idx) or 'done': "
                            )
                            if inp.lower() == "done":
                                break
                            tp, tc = inp.split()
                            targets.append(
                                {"target_player_id": tp, "target_char_index": int(tc)}
                            )
                    ...
                    assert state.active_character_index is not None, (
                        "No active character for gravedig"
                    )
                    resolve_gravedig(
                        state, current_player.id, state.active_character_index, indices
                    )
