...
def draw_cards(state: GameState, player_id: str, sources: List[str]):
    ...
    if sources == ["DECK", "DISCARD"]:
        raise ValueError(
            "If drawing from both deck and discard, the discard card must be drawn first."
        )
    ...
    if deck_needed > len(state.deck):
        raise ValueError(
            f"Not enough cards in deck (needed {deck_needed}, has {len(state.deck)})"
        )
    if discard_needed > len(state.discard_pile):
        raise ValueError(
            f"Not enough cards in discard pile (needed {discard_needed}, has {len(state.discard_pile)})"
        )
    ...
    log_event(
        state,
        "DRAW",
        {"sources": sources, "drawn": [c.model_dump() for c in temp_drawn]},
    )
...
def play_card(
    ...
    if character_index is None:
        if player.can_discard_second_face and card.is_face:
            ...
            raise ValueError(
                "Character index required for playing cards in Phase 1 (unless discarding a second face card)"
            )
    ...
    if not card.is_face:
        ...
        if character_index < len(player.characters):
            ...
            character_index == len(player.characters)
            and character_index < state.max_characters
        ):
            ...
            raise ValueError(
                f"Invalid character index or too many characters (max {state.max_characters})"
            )

    log_event(
        state,
        "PLAY_CARD",
        {"card": card.model_dump(), "character_index": character_index},
    )
    end_turn(state)
...
def tap_hero_power(
    ...
    log_event(
        state,
        "TAP_HERO",
        {"char_index": char_index, "suit": char.suit, "rank": char.rank},
    )

    if char.suit == Suit.CLUBS:
        if not target_info:
            raise ValueError("Target info required for Clubs power")
        targets = target_info.get(
            "targets", []
        )  # List of {'target_player_id': ..., 'target_char_index': ...}
    ...
    if (
        is_turn
        and not state.dug_cards
        and state.turn_subphase not in ["SHOPPING", "SHOP_FREE_BUY", "GRAVEDIGGING"]
    ):
        end_turn(state)
...
def perform_action(
    ...
    top_n_cards: int,
    action_suit: Suit,
    dug_indices: Optional[List[int]] = None,
    target_info: Optional[Dict] = None,
):
    """
    ...
    total_rank = sum(
        10 if c.is_ace else c.rank for c in action_cards if c.suit == action_suit
    )
    state.action_taken_this_turn = True
    state.active_character_index = (
        char_index  # Set this as the active character for this turn
    )
...
def apply_face_strike(
    ...
    # Consistency check
    if (
        state.active_character_index is not None
        and char_index != state.active_character_index
    ):
        raise ValueError("Recursive actions must use the same character")
    ...
    if (
        not removed
        and not state.cards_removed_this_turn
        and not state.character_tapped_this_turn
    ):
        log_event(
            state,
            "CHARACTER_DEATH",
...
def end_turn(state: GameState):
    ...
    if state.phase == 2 and not state.is_over:
        if not (
            state.action_taken_this_turn
            or state.cards_removed_this_turn
            or state.character_tapped_this_turn
        ):
    ...
    log_event(
        ...
        "TURN_START",
        {
            "player_id": state.players[state.current_turn_index].id,
            "subphase": state.turn_subphase,
        },
    )
